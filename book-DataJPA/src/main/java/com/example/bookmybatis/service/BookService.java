...
//    } //MyBatis 용
    ...
    public List<Book.Simple> findCondBooks(Book.Create bookFrom) {
        ...
        List<Book.Simple> list = new ArrayList<>();
//        for(BookEntity bookEntity2 : bookRepository.findCond(bookEntity)) { //MyBatis 용
        for(BookEntity bookEntity2 : bookRepository.findCond(bookFrom.getName(), bookFrom.getPublisher())) {
...
//        bookRepository.update(bookEntity); //MyBatis 용
        bookRepository.save(bookEntity); //DataJPA 용
    }
